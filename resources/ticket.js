...
!function(window){
  ...
  window.GrooveWidget = {
    ...
    setup: function() {
      ...
      this.container.className = 'closed ' + this._options.position + (this._options.chatOnly ? ' chat-only' : '');
      ...
      this.endChatButton = document.getElementById('gw-end-chat');
    ...
    createContainerMarkup: function() {
      ...
      return '<div id="groove-button">'
           ...
           + '  </span></a>'
           + '  <a id="gw-end-chat" href="#" onclick="GrooveWidget.endChat(); return false;">'
           + '    End Chat'
           + '  </a>'
           + '</div>'
           + '<div class="iframe">'
    ...
    onMessage: function(event) {
      ...
      switch (eventData.name) {
        ...
        case 'chat-online':
          GrooveWidget.setOnline(true);
          break;
        case 'chat-accepted':
          GrooveWidget.setChat(eventData.agent.name); break;
        case 'chat-offline':
          GrooveWidget.setOnline(false);
          GrooveWidget.setChat(false);
          break;
        case 'ticket-offline':
    ...
    trigger: function(){
      if (window.jQuery) {
        try {
          var $w = jQuery(window), args = [].slice.apply(arguments);
          args.unshift('groove.chat.' + args.shift()); // 'ready' -> 'groove.chat.ready'
          $w.trigger.apply($w, args);
        } catch(e) { /*jquery here is weird, doing nothing*/ }
      }
    },

    setChat: function(status){
    ...
    updateHeaderText: function() {
      ...
      } else if (this.chatStatus) {
        this.setHeaderText(this._options.chattingWith + ' ' + this.chatStatus);
    ...
    updateClassName: function() {
      this.container.className = (this.opened ? 'opened' : 'closed') + ' ' + this._options.position + ' ' + this.state + ' ' + this.panel + (this._options.chatOnly ? ' chat-only' : '') + (this.zoomLevel >= 0 ? ' zoom-level-' + this.zoomLevel : '');
    },
    ...
    setOnline: function(online) {
      ...
      // Update header when online state changes.
      this.updateHeaderText();

      if (this._options.onlyShowOnline) {
        this[ online ? 'show' : 'hide' ]();
      }

      // Trigger required event.
      this.trigger(online ? 'online' : 'offline');
      this.postIframeMessage('isVisible');
    },
    ...
    isIOSDevice: function() {
      return navigator.userAgent.match(/(iPhone);.*CPU.*OS [6,7]_\d/i);
    },
    ...
    endChat: function() {
      this.postIframeMessage('endChat');
    },
...
GrooveWidget.init({"position":"right","onlineHeaderText":"Feedback & Support","onlineHeaderTextOpen":"Need help?","offlineHeaderText":"Need help?","offlineHeaderTextOpen":"Need help?","chattingWith":"Chatting with","url":"https://studentloanhero.groovehq.com/widgets/503430ae-48cf-4d68-a3b1-6be91f0c2a07/ticket/init","stylesheet":"https://studentloanhero.groovehq.com/assets/groove.widget-863b6d5d91a43fe11df61984061af681.css","customization_stylesheet":"https://studentloanhero.groovehq.com/widgets/503430ae-48cf-4d68-a3b1-6be91f0c2a07/ticket/customization.css","height":"70px","origin":"https://studentloanhero.groovehq.com","enduser_name":"","enduser_email":"","onlyShowOnline":false,"poweredByGrooveText":"Powered by Groove","chatOnly":false});
